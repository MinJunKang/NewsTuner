...
## 원문을 한 번에 복사하기 (iOS 단축어)

난이도에서 **원문**을 고르면 기사를 붙여넣어 읽을 수 있습니다. 이때 사파리에서
본문을 손으로 선택하는 게 번거로우면, 단축어로 그 과정을 없앨 수 있습니다.

단축어 앱에서 새 단축어를 만들고 액션 세 개를 넣습니다.

1. **웹페이지에서 기사 가져오기** (Get Article from Web Page)
2. **기사 세부사항 가져오기** → 항목을 **본문**(Body)으로
3. **클립보드에 복사** (Copy to Clipboard)

그다음 단축어 정보에서 **공유 시트에 표시**를 켜고, 입력 유형을 URL과
웹페이지로 둡니다.

쓸 때는 이렇게 됩니다.

```
사파리에서 기사 열기 → 공유 → 방금 만든 단축어
  → News Tuner 에서 "클립보드에서 가져오기" 한 번
```

본문만 깔끔하게 넘어오고, 선택할 필요가 없습니다. 액션 이름은 iOS 버전에 따라
조금씩 다를 수 있으니 비슷한 이름으로 찾으세요.

단축어가 번거로우면 사파리의 **읽기 도구**(주소창 왼쪽 aA)를 먼저 켜세요.
본문만 남아서 전체 선택이 훨씬 쉬워집니다.
