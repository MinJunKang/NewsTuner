...
아래 표에서 **실측**은 앱의 사용량 기록(설정 탭 → 사용량 기록)에 찍힌 토큰 수에
2026년 8월 단가(3.7 Flash $0.75/$3.75, 3.5 Flash-Lite $0.30/$2.50 per 1M)를
곱한 값입니다. 추정은 아직 실측 표본이 없는 항목입니다.

| 동작 | 1회 | |
|---|---|---|
| **기사 찾기 — 피드+전문 경로** | **약 $0.009** (실측) | 모델 호출 **1번**이 전부입니다. 검색을 안 하므로 그라운딩 요금도 0 |
| 기사 찾기 — 검색 경로 | 약 $0.02~0.05 (추정) | 목록 검색과 집필 검색이 각각 붙고, 검색 결과가 입력에 통째로 들어갑니다 |
| 목록 실패 후 재시도 | 약 $0.018 (실측) | 큰 모델 + medium thinking. **기사를 못 얻고도** 나갑니다 |
| 단어 클릭 | 약 $0.001 (추정) | |
| 문장 해석 | 약 $0.0013 (추정) | |
| 대화 1턴 | 약 $0.005 (추정) | |

기사 한 편의 실측 내역은 입력 4,633 · 생각 0 · 출력 1,400 토큰입니다. 집필 호출은
`thinkingLevel: "low"` 에서 생각 토큰을 **0** 으로 쓰므로, 여기서 더 깎을 것은 없습니다.

그래서 비용을 줄이는 가장 큰 레버는 thinking 이 아니라 **경로**입니다. 피드가 있는
매체를 고르고 전문 추출이 성공하면 한 기사가 호출 한 번으로 끝나지만, 피드가 없어
검색 경로로 떨어지면 두세 배가 되고, 목록마저 실패하면 기사 없이 $0.018 이 나갑니다.
매체를 추가할 때 RSS 피드 유무를 먼저 확인해야 하는 이유입니다.

피드+전문 경로 기준으로 매일 1기사면 월 $0.3 안팎, 단어·대화를 곁들여도 월 $1 안쪽,
하루 3기사씩 많이 써도 월 $2 정도입니다.
